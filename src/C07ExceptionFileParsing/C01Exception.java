...
//
////        예외처리 : 예외가 발생할 것으로 예상되는 코드를 try를 감싼다.
//        try {
...
//            e.printStackTrace(); // 이와 같은 방식으로 시스템 로그를 기록.
...
//            e.printStackTrace();
//        } finally { // 예외가 발생하든 하지 않든 무조건 실행되는 구문
...
//        2) throws를 통해서 checked 예외처리를 위임받게되면, 호출하는 쪽으로 예외처리.
        String result = "";
...
//            예외는 기본적으로 해당 메서드를 호출한 쪽으로 전파
            throw new NoSuchElementException("그러한 요소가 없습니다.");
        } else {
...
//        방법1(통상적인 방법). checked를 try를 catch한 이후에 unchecked 예외를 다시 throw
        String text = null;
        try {
